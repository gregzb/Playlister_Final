...
export const updatePlaylistInteractions = exceptionWrapper(async (playlist: Playlist): Promise<ErrT | SuccessT & {
    playlist: Playlist
}> => {
    const res = await fetch(`${baseUrl}/playlist/interactions/${playlist._id}`, {
        method: "PUT",
        credentials: "include",
        headers: {
            "Content-Type": "application/json"
